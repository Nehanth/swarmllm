...
//   NODE_PATH=... node tests/e2e/state_synth.mjs [--model f.gguf] [--prompt-len 60] [--tokens 12] [--flash 0|1]
...
async function pageMain({ plen, ntok, flash }) {
  ...
  const eng = await Qwen35Engine.create({ device, meta: G.meta, layerRange: [0, L], hasEmbed: true, hasHead: true, vocab: G.tensors[GGML_EMBED].shape[0],
    maxSeq: 1024, batchCols: 16, coopRowsB: 1, coopWG: 64, attnFlash: flash,
...
const res = await page.evaluate(pageMain, { plen: +arg("prompt-len", 60), ntok: +arg("tokens", 12), flash: arg("flash", "1") !== "0" });
