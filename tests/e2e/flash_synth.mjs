...
//   NODE_PATH=... node tests/e2e/flash_synth.mjs [--model f.gguf] [--prompt-len 300] [--tokens 16] [--max-seq 2048]
...
async function pageMain({ plen, ntok, maxSeq }) {
  ...
  const mk = async (attnFlash) => Qwen35Engine.create({ device, meta: G.meta, layerRange: [0, L], hasEmbed: true, hasHead: true, vocab: G.tensors[GGML_EMBED].shape[0],
    maxSeq, batchCols: 16, coopRowsB: 1, coopWG: 64, attnFlash,
  ...
  const B = await mk(true); const b = await run(B);
  say(`engine.flash: old ${A.flash}, new ${B.flash} (split ${B.faSplit} positions, ${B.faSplits} splits)`);
...
const res = await page.evaluate(pageMain, { plen: +arg("prompt-len", 300), ntok: +arg("tokens", 16), maxSeq: +arg("max-seq", 2048) });
