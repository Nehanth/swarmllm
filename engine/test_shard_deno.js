...
import { parseSafetensors, makeTokenizer, BelloEngine, argmax, fetchModelShard, shardTensorNames } from "./engine.js";
const dir = new URL(".", import.meta.url).pathname;
const cfg = JSON.parse(await Deno.readTextFile(dir + "model/config.json"));
const golden = JSON.parse(await Deno.readTextFile(dir + "golden.json"));
const tok = makeTokenizer(JSON.parse(await Deno.readTextFile(dir + "model/tokenizer.json")));
...
const localTensors = parseSafetensors((await Deno.readFile(dir + "model/model.safetensors")).buffer);
