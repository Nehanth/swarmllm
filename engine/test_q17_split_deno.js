...
const cfg = JSON.parse(await Deno.readTextFile(dir + "qwen17/config.json"));
const golden = JSON.parse(await Deno.readTextFile(dir + "golden_qwen17.json"));
const tok = makeTokenizer(JSON.parse(await Deno.readTextFile(dir + "qwen17/tokenizer.json")));
const raw = await Deno.readFile(dir + "qwen17/model.gguf");
