...
// Usage (GB10 / Deno):
//   deno run --unstable-webgpu --allow-read --allow-env bench_deno.js
// Env:
...
import { BelloEngine, makeTokenizer, argmax } from "./engine.js";
import { Qwen35Engine } from "./qwen35.js";
import { parseGGUFHeader, ggufWeights, qwen35Weights, tokenizerFromGGUF } from "./gguf.js";
...
if (MODEL === "q38") {
  const readAt = await openFile(Deno.env.get("GGUF") || "q38/model.gguf");
  ...
  tok = makeTokenizer(JSON.parse(await Deno.readTextFile("qwen/tokenizer.json")));
  const cfg = JSON.parse(await Deno.readTextFile("qwen/config.json"));
