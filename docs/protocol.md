...
| `ai-progress {pct}` / `ai-hostprog {all, layers}` | worker → host / host → all | download progress: the host relays every device's percentage (`all`, by name) and the range each one is loading (`layers`) every 600 ms while any plan loads; every screen draws it on that peer's card. A final frame with every device at 100 clears the cards |
...
| `ai-genstart` / `ai-token` / `ai-gendone {stats}` | host → all | mirror the question and streamed answer to every screen. `stats` is the tok/s line, `failed: …`, or `stopped: <name> left (layers a–b)` when a chain member left mid-answer. Under `ai-visibility` `host`/`asker`, the text goes only to the allowed screens; the others get `ai-genstart`/`ai-gendone` with `hidden: true` (no `ai-token`), so every Send box still locks and unlocks |
...
| `online` | the current plan is served by every chain member; questions are accepted. A newer plan may be **pending** (a join, or the re-deal button): its newcomers download in the background while this plan keeps answering, the status line says who is loading what, Send stays open |
...
| `redealing` | the room is switching plans. After a **leave** it is a reload: the departed range goes to survivors (from the Cache API where they have it), every screen shows the leaver's name and the per-device download on the peer cards, questions wait. After a **join** it is the switch to the pending plan once the newcomers are ready: survivors only narrow (a sub-range of what they hold, no reload) or re-link, the host narrows or reloads its own range from cache; typically under a second. Back to `online` when every chain member of the new plan reported `ai-ready {v}` |
...
All of this runs on the AI host (the boss: `aiStartAnywhere` picks the biggest pledge, which may be a joiner, as in a room a phone created and a laptop joined). The planner, the watchdogs and `peerGone` are wired on `ai.role === "host"`. A boss that joined has a link to the creator only, so it dials every roster member itself before the first plan (`bossAdopt`: `ensureLink` plus the roster's name and pledge copied onto the link) and learns about joins and leaves from the creator's `roster` broadcasts; a boss that is also the creator learns them from `hello` and the link `close` directly. A joiner without WebGPU is never dialed by a joined boss, so it cannot ask questions in that room (it can when the creator is the boss).

A **join** while the model runs gets a fair share at the next plan, prepared in the background. The planner places the newcomer at a seam between two survivors (or after the last one) and only shrinks survivors, so the switch needs no reload (`narrow`: the engine serves a sub-range of the layers it holds, byte-identical to a fresh load; `tests/test_narrow.js`). Sequence: `ai-load {v}` to the newcomers only, the old chain keeps answering (`ai.pending`); when every newcomer reported `ai-ready {v}` and no answer is in flight, `flipPlan` sends `ai-load {v}` to the survivors (narrow or re-link) and `ai-wait` to devices that lost every layer, narrows or reloads the host, and the room is `online` on the new plan. A question that starts during the switch is refused with `ai-busy`; one that starts before it keeps the old chain and the switch waits for `ai-gendone`. Every answer starts from `ai-reset` and position 0 (single turn), so no state migrates across a plan. The manual **re-deal** button runs the same planner (a device that would grow reloads its new range from cache). Pledge changes after start do not re-plan in v1.
...
i. **narrow-only join** (after the steps below, when `prev` is given and its tiles still cover `[0, L)`, i.e. nobody left): every survivor keeps a sub-range of its old tile; each newcomer, with its proportional share `k` (capped by its memory), takes `t` layers from the top of one neighbour and `b = k − t` from the bottom of the next (or `k` from the top of the last), at the seam whose result lands closest to the proportional deal; every device keeps at least one layer. The chain order becomes the tile order. When no seam can free a newcomer's share the proportional deal below applies as is (survivors may reload). Result flag: `narrow`.
