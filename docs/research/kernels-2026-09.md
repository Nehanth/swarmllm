...
| Fused attention glue (`attn_glue`: qsplit + q/k `head_norm` + partial rope) | `attnGlue: false` or `engine.attnGlue = false` (bit-identical); room `?fuse=0` turns off all three fusions | −4 dispatches per attention layer (−64 per token on the 27B, ~2–4.5 ms on Metal at 32–71 µs each, item 5); the norms also move from one thread per head to a workgroup | `node tests/e2e/fusion_synth.mjs` (identity + ms/token); on a GPU, decode tok/s with and without |
| Fused DeltaNet delta rule + gated norm (`dn_delta_gn`, decode) | `dnFuse: false` or `engine.dnFuse = false` (bit-identical) | −1 dispatch per DeltaNet layer (−48 per token on the 27B) and one fewer round trip of the head outputs through global memory | `node tests/e2e/fusion_synth.mjs`; on a GPU, decode tok/s with and without |
| Batched attention for verify / prefill (`attn_scores_mc`, `attn_softmax_wg_mc`, `attn_out_mc`) | `attnMC: false` or `engine.attnMC = false` (bit-identical; off automatically when the workgroup softmax is off) | 3 dispatches per attention layer instead of 3 per column: −9 × 16 = −144 per K=3 verify, −720 per 16-column prefill chunk; columns also run in parallel instead of in turn | `node tests/e2e/fusion_synth.mjs`; on a GPU, spec tok/s and prefill tok/s with and without |
